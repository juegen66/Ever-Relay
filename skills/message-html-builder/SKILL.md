...
description: Create self-contained single-file HTML for chatbot messages, inline cards, embeddable widgets, compact dashboards, and interactive reply content. Use when Codex is answering a chatbot user and determines that the response would be clearer, more visual, or more interactive as HTML than as plain text, especially for charts, data summaries, comparisons, process visuals, rich cards, or other vivid inline presentations. Always style with Tailwind CSS loaded from CDN. When the request involves charts, trends, proportions, comparisons, or other data visualizations, load ECharts from CDN and use it for the chart implementation.
---
...
- Pass the `html` argument as raw document content, not as a JSON-stringified or double-escaped string full of `\n`.
