...
export function HeroSection() {
  return (
    <section className="relative flex min-h-screen flex-col items-center justify-center overflow-hidden px-6 pt-16">
      ...
      {/* Badge */}
      <div className="animate-hero mb-6 flex items-center gap-2 rounded-full border border-white/[0.08] bg-white/[0.04] px-4 py-1.5">
        <Sparkles className="h-3.5 w-3.5 text-[#007aff]" />
        <span className="text-[13px] text-white/60">Your Desktop, Reimagined</span>
      </div>

      {/* Headline */}
      <h1 className="animate-hero-delay-1 max-w-3xl text-center text-5xl font-bold leading-[1.1] tracking-tight text-white md:text-7xl">
        A Complete{" "}
        <span
          className="bg-clip-text text-transparent animate-shimmer"
          style={{
            backgroundImage: "linear-gradient(110deg, #007aff, #5856d6, #af52de, #007aff)",
      ...
      {/* Subtitle */}
      <p className="animate-hero-delay-2 mt-6 max-w-xl text-center text-[17px] leading-relaxed text-white/50">
      ...
      <div className="animate-hero-delay-3 mt-8 flex items-center gap-4">
        <a
          href="#register"
          className="group flex items-center gap-2 rounded-full bg-[#007aff] px-7 py-3 text-[15px] font-medium text-white transition-all hover:bg-[#0066d6] hover:shadow-lg hover:shadow-[#007aff]/20"
        >
          Create Account
        ...
        <Link
          href="/desktop"
          className="rounded-full border border-white/[0.12] bg-white/[0.04] px-7 py-3 text-[15px] font-medium text-white transition-all hover:bg-white/[0.08]"
        >
          Try Now
        </Link>
      </div>
      ...
      <div className="animate-hero-delay-3 mt-16 w-full max-w-5xl animate-float">
        <div
          ...
          style={{
            boxShadow: "0 40px 100px -20px rgba(0,122,255,0.15), 0 0 40px rgba(0,0,0,0.4)",
          }}
        >
          <Image
          ...
          {/* Fake menu bar */}
          <div className="absolute top-0 left-0 right-0 h-6 bg-black/30 backdrop-blur-xl flex items-center px-4">
